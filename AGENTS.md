...
GarraIA is a multi-crate Rust workspace (`edition = "2024"`, `rust-version = "1.94"`). The main binary is `garraia-cli`; the HTTP/WS gateway lives in `garraia-gateway`.
...
- The workspace uses `edition = "2024"` and declares `rust-version = "1.94"` (GAR-895 — bumped for wasmtime 46 and wiggle-macro 46.0.2 toolchain requirements). The VM ships with Rust 1.94+.
