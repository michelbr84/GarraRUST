- **O detector de loop de ferramenta avisa o modelo uma vez antes de abortar
  ...
  de outro, aborta como antes, com a mensagem do #1318. O efeito colateral
  nao muda: a chamada em loop continua rodando no maximo duas vezes por
  tarefa, e o custo extra e no maximo uma volta de LLM. O aviso so e rearmado
  por uma mensagem nova do usuario, nunca pelo reset do teto por turno, e nao
  ha chave de config para desliga-lo. Dentro de `tool_program` (e no
