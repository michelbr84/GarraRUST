- **Circuit breaker para ferramentas que falham repetidamente (#1417).** No
  ...
  caminho fora das raizes, `repo_search` sem repositorio) poe a ferramenta em
  pausa ate o fim do turno; timeout abre um cooldown que dobra a cada timeout
  seguido (15s ate 120s) e atravessa turnos; erro generico so abre depois de
  tres iguais no turno. Em pausa, a chamada nao roda e o modelo recebe o
  motivo estruturado ("temporariamente indisponivel nesta conversa
  (`no_roots` | `outside_roots` | `no_repository` | `timeout` |
  `repeated_error`)") com a instrucao de nao repetir. Sucesso fecha o breaker
