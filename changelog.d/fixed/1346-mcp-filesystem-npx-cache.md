- **O MCP `filesystem` deixa de ficar preso num cache do npx corrompido e para de
  ...
  vez de baixar o build mais novo do registry a cada cache frio; um `mcp.json`
  existente nunca e reescrito, e o novo check `mcp.filesystem_pinned` do
  `/api/diagnostics` avisa quem ficou sem versao com os `args` exatos para colar.
  ...
  entrada e tenta de novo uma vez: comando `npx`, caminho canonico dentro do
  cache que o proprio gateway deu ao filho, nunca symlink, `package.json` do
  pacote configurado, uma vez por servidor por processo. Esgotados os
  ...
  respondia `no_mcp_configured`), com `status`, `cause` e `last_error`, e o
  check `mcp.servers` do diagnostico traz o proximo passo por causa.
