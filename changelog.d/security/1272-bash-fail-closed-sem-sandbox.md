...
- **O container do sandbox passa a ser fronteira de verdade (#1272).** `docker
  ...
  canonico e absoluto do diretorio de trabalho; relativo, inexistente ou com
  `:` agora e recusa fail-closed, em vez de rodar sem o mount.
- **`git_diff` e `code_review` nao executam programa plantado no repositorio
  ...
  todo `filter.<driver>` da config anulado e `GIT_CONFIG_NOSYSTEM=1`; e
