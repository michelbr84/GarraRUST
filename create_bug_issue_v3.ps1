...
O gateway faz panic fatal na inicialização devido ao axum Router invalidar rotas com segmentos queبدأو com : (dois pontos).

Erro: Path segments must not start with ':'. For capture groups, use '{capture}'.

## Causa Raiz

A rota /:id esta sendo usada em vez de /{id}. O axum v0.7+ requer o formato de captura /{key}.
...
- Gateway inicia sem panic
"@

$mutation = @"
{
  "query": "mutation { issueCreate(input: { teamId: \"" + $teamId + "\", title: \""" + $title + "\" , description: \""" + $description + "\" , labels: [\"bug\", \"urgent\"] }) { success issue { id identifier title } } }"
}
"@

try {
    $result = Invoke-RestMethod -Uri "https://api.linear.app/graphql" -Method Post -Headers $headers -Body $mutation
