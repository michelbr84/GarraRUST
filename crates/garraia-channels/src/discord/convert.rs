...
/// Convert a serenity Discord message into an GarraIA `Message`.
///
...
/// Convert a reaction add event into an GarraIA `Message` with `Reaction` content.
pub fn reaction_to_garraia(reaction: &serenity_model::Reaction, channel_id_str: &str) -> Message {
...
/// Convert an GarraIA `MessageContent` into a plain text string suitable
/// for sending as a Discord message.
pub fn garraia_content_to_text(content: &MessageContent) -> String {
    match content {
...
mod tests {
    ...
    fn text_content_converts_to_string() {
        ...
        assert_eq!(garraia_content_to_text(&content), "Hello, world!");
    }
    ...
    fn image_with_caption_converts_correctly() {
        ...
        let result = garraia_content_to_text(&content);
