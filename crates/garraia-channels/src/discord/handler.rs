...
/// Serenity event handler that bridges Discord events into GarraIA `ChannelEvent`s.
pub struct DiscordHandler {
    /// Broadcast sender for emitting channel events to subscribers.
    event_tx: broadcast::Sender<ChannelEvent>,

    /// The channel identifier string used in GarraIA messages.
    channel_id: String,
...
impl EventHandler for DiscordHandler {
    ...
    async fn message(&self, ctx: Context, msg: SerenityMessage) {
        ...
        let garraia_msg = convert::discord_message_to_garraia(&msg, &self.channel_id);
        self.emit(ChannelEvent::MessageReceived(garraia_msg));
    ...
    async fn reaction_add(&self, _ctx: Context, reaction: serenity_model::Reaction) {
        let garraia_msg = convert::reaction_to_garraia(&reaction, &self.channel_id);
