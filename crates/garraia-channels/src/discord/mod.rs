//! Discord channel implementation for GarraIA.
//!
...
/// the GarraIA `ChannelEvent` system.
...
impl Channel for DiscordChannel {
    ...
    async fn send_message(&self, message: &Message) -> Result<()> {
        ...
        let channel = serenity_model::ChannelId::new(discord_channel_id);
        let text =
            convert::to_discord_markdown(&convert::garraia_content_to_text(&message.content));
...
mod tests {
    ...
    async fn send_message_without_connection_fails() {
        ...
        let channel = DiscordChannel::new(test_config(), on_msg);
        let msg = garraia_common::Message::text(
            garraia_common::SessionId::from_string("test"),
            garraia_common::ChannelId::from_string("discord"),
            garraia_common::UserId::from_string("user"),
