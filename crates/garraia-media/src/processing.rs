...
impl MediaProcessor {
    ...
    fn validate_path(&self, path: &Path, is_output: bool) -> Result<PathBuf> {
        ...
        let canonical_path = if is_output && !path.exists() {
            ...
            let canonical_parent = std::fs::canonicalize(parent).map_err(|_| {
                garraia_common::Error::Security(format!(
            ...
            if let Some(file_name) = path.file_name() {
                ...
                {
                    return Err(garraia_common::Error::Security("Invalid filename".into()));
                }
            }
    ...
    pub async fn convert_audio(
        ...
        if !status.success() {
            return Err(garraia_common::Error::Media(
...
mod tests {
    ...
    async fn test_allowed_paths_validation() -> Result<()> {
        let temp_dir = std::env::temp_dir().join("garraia_media_test_allowed_2");
        if temp_dir.exists() {
    ...
    async fn test_disallowed_paths_validation() -> Result<()> {
        let temp_dir = std::env::temp_dir().join("garraia_media_test_denied_2");
        if temp_dir.exists() {
            std::fs::remove_dir_all(&temp_dir).unwrap();
        }
        std::fs::create_dir_all(&temp_dir).unwrap();
        ...
        let other_dir = std::env::temp_dir().join("garraia_media_test_other_2");
        if other_dir.exists() {
    ...
    async fn test_path_traversal_prevention() -> Result<()> {
        let temp_dir = std::env::temp_dir().join("garraia_media_test_traversal_2");
        if temp_dir.exists() {
            std::fs::remove_dir_all(&temp_dir).unwrap();
        }
        std::fs::create_dir_all(&temp_dir).unwrap();
    ...
    async fn test_dangling_symlink_rejection() -> Result<()> {
        ...
        let temp_dir = std::env::temp_dir().join("garraia_media_test_symlink_allowed_2");
        let forbidden_dir = std::env::temp_dir().join("garraia_media_test_symlink_forbidden_2");
