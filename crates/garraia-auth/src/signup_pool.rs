...
impl std::fmt::Debug for SignupPool {
    ...
    ///
    /// `SignupPool` can only be constructed via `from_dedicated_config`, which
    /// requires a live Postgres connection (`garraia_signup` role). There is no
    /// way to obtain a `SignupPool` instance for a unit test without
