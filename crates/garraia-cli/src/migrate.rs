...
pub fn migrate_openclaw(
    ...
    import_skills(&source_dir, garraia_dir, &mut report);
    import_conversations(&source_dir, &mut report);
    import_channels(&source_dir, garraia_dir, &mut report);
    import_credentials(&source_dir, &mut report);

    Ok(report)
}

fn import_skills(source_dir: &Path, garraia_dir: &Path, report: &mut MigrationReport) {
    let skills_src = source_dir.join("skills");
    if !skills_src.exists() {
        return;
    }

    let skills_dst = garraia_dir.join("skills");
...
fn import_conversations(source_dir: &Path, report: &mut MigrationReport) {
    ...
    for dir_name in &conv_dirs {
        ...
        for entry in entries {
            ...
            // require re-embedding. Users start fresh with GarraIA.
        }
    }
...
fn import_channels(source_dir: &Path, garraia_dir: &Path, report: &mut MigrationReport) {
    ...
    let garraia_config_path = garraia_dir.join("config.yml");
    let mut garraia_config: serde_yaml::Value = if garraia_config_path.exists() {
        let existing = std::fs::read_to_string(&garraia_config_path).unwrap_or_default();
    ...
    let dest_channels = garraia_config
        .as_mapping_mut()
        .unwrap()
    ...
    for (name, channel_value) in channels {
        ...
        if dest_map.contains_key(&name_key) {
            println!("  channel '{name_str}' already exists in GarraIA config, skipping");
    ...
    if !report.dry_run
        ...
        && let Ok(yaml_str) = serde_yaml::to_string(&garraia_config)
        && let Err(e) = std::fs::write(&garraia_config_path, yaml_str)
    {
        report
            .errors
    ...
    if found_any && !report.dry_run {
        println!(
            "  Note: credential migration is skipped — re-enter API keys \
             via `garraia init` or set environment variables directly."
        );
    }
}
