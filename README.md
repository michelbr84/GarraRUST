...
> **Requisito mínimo (Linux):** os binários publicados exigem **glibc ≥ 2.35** (Ubuntu 22.04+, Debian 12+). Desde a v0.3.3 o build x86_64 é pinado em `ubuntu-22.04` no CI (a v0.3.2, buildada em `ubuntu-latest`/24.04, falhava com ``version `GLIBC_2.39' not found`` em sistemas mais antigos) e o `install.sh` checa a glibc local antes de baixar. Distros mais antigas ou musl (Alpine): compile do source (`cargo install --git https://github.com/michelbr84/GarraRUST garraia`).
