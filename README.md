...
Pre-compiled binaries for Linux (x86_64, aarch64), macOS (Intel, Apple Silicon), and Windows (x86_64) are available on [GitHub Releases](https://github.com/michelbr84/GarraRUST/releases).
...
| **Self-update** | Yes (`garraia update`) | npm | Build from source |

*Benchmarks measured on a 1 vCPU, 1 GB RAM DigitalOcean droplet. [Reproduce them yourself](bench/).*

## Security

GarraIA is built for the security requirements of always-on AI agents that access private data and communicate externally.

- **Encrypted credential vault** - API keys and tokens stored with AES-256-GCM encryption at `~/.garraia/credentials/vault.json`. Never plaintext on disk.
...
- Configure in `config.yml` or `~/.garraia/mcp.json` (Claude Desktop compatible)
...
- Auto-discovery from `~/.garraia/skills/` - injected into the system prompt
...
- **Daemonization** - `garraia start --daemon` with PID management
- **Self-update** - `garraia update` downloads the latest release with SHA-256 verification, `garraia rollback` to revert
- **Restart** - `garraia restart` gracefully stops and starts the daemon
- **Runtime provider switching** - add or switch LLM providers via the webchat UI or REST API without restarting
- **Migration tool** - `garraia migrate openclaw` imports skills, channels, and credentials
- **Interactive setup** - `garraia init` wizard for provider and API key configuration
...
GarraIA looks for config at `~/.garraia/config.yml`:
...
crates/
  ...
  garraia-media/      # Media processing (scaffolded)
...
GarraIA is open source under the MIT license. Join the [Discord](https://discord.gg/aEXGq5cS) to chat with contributors, ask questions, or share what you're building. See [CONTRIBUTING.md](CONTRIBUTING.md) for setup instructions, code guidelines, and the crate overview.

### Current priorities

| Priority | Issue | Description |
|----------|-------|-------------|
| **P0** | [#103](https://github.com/michelbr84/GarraRUST/issues/103) | README and positioning |
...
| **P1** | [#108](https://github.com/michelbr84/GarraRUST/issues/108) | Multi-agent routing |
| **P1** | [#109](https://github.com/michelbr84/GarraRUST/issues/109) | Install script |
| **P1** | [#110](https://github.com/michelbr84/GarraRUST/issues/110) | Linux aarch64 + Windows releases |
| **P1** | [#80](https://github.com/michelbr84/GarraRUST/issues/80) | MCP: HTTP transport, resources, prompts |

Browse all [open issues](https://github.com/michelbr84/GarraRUST/issues) or filter by [`good-first-issue`](https://github.com/michelbr84/GarraRUST/issues?q=label%3Agood-first-issue+is%3Aopen) to find a place to start.
