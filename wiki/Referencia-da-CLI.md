...
| `garraia whatsapp allow <número> [--owner] [--yes]` | Autoriza um número (com código do país) a falar com o GarraIA pelo WhatsApp pessoal, sem precisar de terminal; vale sem reiniciar o gateway (#1345). Exit 64 para `--owner` fora de `isolated-pod` ou num pipe sem `--yes`, 65 para número inválido |
