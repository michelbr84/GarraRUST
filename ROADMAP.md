...
**Status (2026-04-14):** 🟢 **GAR-392 entregue — Fase 3.3 quase fechada; epic GAR-391 ainda aberto.** ADR 0003 + ADR 0005 (com Amendment 2026-04-13) accepted. Crate `garraia-workspace` com **10 migrations**. Crate `garraia-auth` com: `Principal` extractor (Axum) + `RequirePermission` struct method + `Role`/`Action` enums tipados + `fn can()` central (110-case table-driven test) + `SignupPool` newtype análogo ao `LoginPool` + `signup_user` free function + `RedactedStorageError` wrapper (M-3 fechado) + `AuthConfig` em `garraia-config` + 4 endpoints `/v1/auth/{login,refresh,logout,signup}` wired no `AppState` real (feature `auth-v1` REMOVIDA) + métricas Prometheus baseline com bounded enum + **migration 010** fechando os 3 structural gaps (Gap A `GRANT SELECT ON sessions`, Gap B `garraia_signup NOLOGIN BYPASSRLS` separado, Gap C `GRANT SELECT ON group_members`) + **GAR-392 RLS matrix** (plan 0013 path C, 81 cenários table-driven contra pgvector/pg16 real × 3 dedicated roles × 10 FORCE RLS tables × 4 `TenantCtx` variants, oracle SQLSTATE, 5 meta tripwires). Pending: **GAR-391d** (app-layer cross-group matrix via HTTP) deferido ao **plan 0014** — só destrava com a superfície REST `/v1/{chats,messages,memory,tasks,groups,me}` da Fase 3.4. Fechar 391d fecha o epic GAR-391. ADRs 0004 (storage), 0006 (search) e 0008 (docs collab) ainda pendentes.
...
**Atualizado 2026-04-14** após entrega de Fase 2.3 (OTel ✅), schema completo da Fase 3.2 (9 migrations ✅), Fase 3.3 ADR 0005 ✅, GAR-391a/b/c (crate `garraia-auth` com verify path real + extractor + endpoints ✅) e GAR-392 (RLS matrix 81 cenários ✅, plan 0013 path C). Epic **GAR-391 permanece aberto** — GAR-391d (app-layer cross-group matrix via HTTP) foi deferido ao plan 0014 e só destrava quando a superfície REST `/v1/{chats,messages,memory,tasks,groups,me}` nascer na Fase 3.4.

Quando retomar execução, priorizar **nesta ordem**:

1. **GAR-391d — App-layer cross-group matrix** ⏸ **deferida para Fase 3.4** (plan 0014, futuro). Verificação empírica 2026-04-14 mostrou que 15 de 18 endpoints REST-de-recurso (`/v1/chats`, `/v1/messages`, `/v1/memory`, `/v1/tasks`, `/v1/groups`, `/v1/me`) ainda não existem em `garraia-gateway` — sem superfície HTTP para exercitar a dimensão `Relationship` via `Principal` extractor + `RequirePermission`. O design doc [`docs/superpowers/specs/2026-04-14-gar-391d-392-authz-suite-design.md`](docs/superpowers/specs/2026-04-14-gar-391d-392-authz-suite-design.md) permanece como estado-alvo do plan 0014 quando os handlers nascerem. Fechar GAR-391d fecha o epic GAR-391.
2. **Fase 3.4 — API REST `/v1` OpenAPI** — precondição para 391d e para qualquer fluxo de produto multi-tenant. Materializa os handlers cobertos pela matriz app-layer.
