...
**Status (2026-04-13):** 🟢 **Schema completo + auth skeleton + verify path real entregues.** ADR 0003 ([GAR-373](https://linear.app/chatgpt25/issue/GAR-373)) e ADR 0005 ([GAR-375](https://linear.app/chatgpt25/issue/GAR-375)) accepted. Crate `garraia-workspace` materializado com **8 migrations**. Crate `garraia-auth` agora com `verify_credential` real (Argon2id + PBKDF2 dual-verify + lazy upgrade transacional sob `FOR NO KEY UPDATE OF ui` + constant-time anti-enumeration via `DUMMY_HASH` em build.rs + audit_events em todos os terminais + JWT HS256 access token) e endpoint `POST /v1/auth/login` sob feature `auth-v1` no `garraia-gateway`, validando 401 byte-identical em todos os modos de falha. **391b shipped com escopo reduzido** (refresh tokens + `SessionStore::issue` no endpoint + `create_identity` movidos para 391c) por gaps estruturais surfacing empíricos: o `garraia_login` role não tem SELECT em sessions nem INSERT em user_identities — design decisions deliberadas que isolam o login flow do refresh/signup flows. Próximo marco: **GAR-391c** (extractor Axum + `RequirePermission` + wiring no `AppState` + refresh/logout/signup endpoints + migration 010 com SELECT on sessions). ADRs 0004 (storage), 0006 (search) e 0008 (docs collab) ainda pendentes.
...
- [ ] **Refresh tokens + `SessionStore::issue` no endpoint** — deferido para 391c (gap estrutural: `garraia_login` precisa de SELECT em sessions, vai junto com o refresh endpoint).
- [ ] **`create_identity` real + signup endpoint** — deferido para 391c (gap estrutural: `garraia_login` precisa de INSERT em user_identities, vai junto com signup pool design).
