...
**Status (2026-04-13):** 🟢 **GAR-391c completo — Fase 3.3 quase fechada.** ADR 0003 + ADR 0005 (com Amendment 2026-04-13) accepted. Crate `garraia-workspace` com **9 migrations**. Crate `garraia-auth` com: `Principal` extractor (Axum) + `RequirePermission` struct method + `Role`/`Action` enums tipados + `fn can()` central (110-case table-driven test) + `SignupPool` newtype análogo ao `LoginPool` + `signup_user` free function + `RedactedStorageError` wrapper (M-3 fechado) + `AuthConfig` em `garraia-config` + 4 endpoints `/v1/auth/{login,refresh,logout,signup}` wired no `AppState` real (feature `auth-v1` REMOVIDA) + métricas Prometheus baseline com bounded enum + **migration 010** fechando os 3 structural gaps (Gap A `GRANT SELECT ON sessions`, Gap B `garraia_signup NOLOGIN BYPASSRLS` separado, Gap C `GRANT SELECT ON group_members`). Pending: **GAR-391d / GAR-392** (suite cross-group authz ≥100 cenários) fecha o epic GAR-391. ADRs 0004 (storage), 0006 (search) e 0008 (docs collab) ainda pendentes.
...
3. **GAR-392 — Pure RLS matrix** ✅ entregue 2026-04-14 (plan 0013 path C). 81 cenários table-driven contra pgvector/pg16 real exercitam os 3 dedicated roles (garraia_app/login/signup) × 10 FORCE RLS tables × 4 `TenantCtx` variants, com oracle SQLSTATE distinguindo `InsufficientPrivilege`/`PermissionDenied`/`RlsFilteredZero`/`RowsVisible`. 5 tripwires meta garantem non-regressão silenciosa. Epic **GAR-391 permanece aberto**.
4. **GAR-391d — App-layer cross-group matrix** ⏸ **deferida para Fase 3.4** (plan 0014, futuro). Verificação empírica 2026-04-14 mostrou que 15 de 18 endpoints REST-de-recurso (`/v1/chats`, `/v1/messages`, `/v1/memory`, `/v1/tasks`, `/v1/groups`, `/v1/me`) ainda não existem em `garraia-gateway` — sem superfície HTTP para exercitar a dimensão `Relationship` via `Principal` extractor + `RequirePermission`. O design doc [`docs/superpowers/specs/2026-04-14-gar-391d-392-authz-suite-design.md`](docs/superpowers/specs/2026-04-14-gar-391d-392-authz-suite-design.md) permanece como estado-alvo do plan 0014 quando os handlers nascerem.
