...
- [x] `GET /v1/search?...&types=tasks` — plan 0191 / [GAR-707](https://linear.app/chatgpt25/issue/GAR-707), implementado 2026-05-26 (Florida). Slice 6: `tasks.title || ' ' || coalesce(tasks.description_md, '')` FTS via `to_tsvector('simple', ...)` (no new migration); group scope only; deleted tasks excluded; `from_date`/`to_date`/`author_id` filters supported; result `type: "task"`, `excerpt` = title, `kind` = status.
