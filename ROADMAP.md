...
>
> **Última atualização:** 2026-09-25 (local America/New_York) — **release v0.4.6** (estabilização): CI sem registry para o MinIO (imagem construída do fonte da tag fixada, #1458); workspace padrão **isolado por sessão** para sessão sem projeto (#1378/#1449); `X-Session-Id` forjado não alcança conversa de canal nem do mobile (#1462); `garraia whatsapp users/remove/owner/unowner` e `link` que termina mostrando o acesso em vigor; diagnóstico que separa "nunca configurado" de "quebrado" (#1437); gate de dogfood manual no runbook (#1439). Inventário das 64 issues e plano de v0.5.0 (Access Policy v2, Capability Registry, WhatsApp no Web Console, onboarding desktop do plan 0363) em `plans/0364`. Anterior (2026-09-22): **release v0.4.5**: o `bash` sai das superfícies sem humano no laço em `standard` — o `garraia mcp-server` e o gateway só o registram dentro de sandbox `docker`/`podman` válido, e poder total só em `isolated-pod` explícito (#1272, Amendment do [ADR 0024](docs/adr/0024-perfis-de-execucao-isolated-pod.md)); aprovação GAR-187 retomada entre turnos em todo canal com humano, com remetente derivado pelo servidor (#1343, #1340); onboarding do WhatsApp pessoal com `garraia whatsapp allow` e releitura a quente de `allow`/`owners` (#1345), com a ponte atualizada no boot depois do `garraia update` (#1373); MCP `filesystem` com versão fixa e recuperação do cache do npx (#1346); `garra_status` nos modos restritos e com os canais do `/api/channels` (#1347); boot que recusa bind exposto sem credencial (#1261) e `config check` em todo boot (#1247); sandbox cobrindo as tools de repositório (#1225); retenção do ledger de runs, `GET /api/runs` e `garraia runs list` (#1227); `garraia-tools`/`garraia-runtime` removidas, 22 crates (#1226); aviso de loop antes de abortar (#1295); correções do smoke de instalação limpa da v0.4.4 (#1369, #1370, #1371, #1372); baseline do Quality Ratchet re-adotado pela ferramenta auditada (#1254). Entregue em dois trens de merge (onda A #1363, 15 PRs; onda B #1375, 11 PRs) mais o re-baseline e o PR de release. Anterior (2026-09-21): **release v0.4.4**: o WhatsApp pessoal passa a funcionar numa instalação nova (a recusa por servidor MCP saiu, #1327; alias `garra` nos instaladores, #1328; toda instrução nomeia o executável real) e chegam os **perfis de execução `standard` e `isolated-pod`** ([ADR 0024](docs/adr/0024-perfis-de-execucao-isolated-pod.md), #1329) — poder total dentro do pod isolado, nada implícito fora; o dono declarado do WhatsApp ganha o piso `code` só em 1:1 e só em `isolated-pod`; o MCP `filesystem` deixa de nascer em `$HOME`. Também: `tool_program` intrínseco com gate por passo (#1226 S-B) e onze achados da revisão pós-merge fechados, aprovação GAR-187 amarrada ao pedido que pausou o turno (#1339), `config check` julgando o bind pelo default do clap (#1261, decisões R5 seguem com o dono). Anterior (2026-09-21): **release v0.4.3**: o Garra entrou no **WhatsApp pessoal por dispositivo vinculado** (`garra whatsapp`, QR, sessão cifrada, bridge Node/Baileys por stdio — [ADR 0023](docs/adr/0023-whatsapp-dispositivo-vinculado.md), #1238); a maior faixa de segurança de uma release (cinco fail-opens do MCP fechados, `gateway.api_key` cobrindo o plano de conversa, jail das file tools, guard de injeção indireta em MCP/`file_read`/dispositivos, `agent.sandbox` com backend `ssh` fail-closed, SSRF pinado, argv injection); runtime com LLM padrão único ([ADR 0022](docs/adr/0022-default-llm-identity.md)), fallback local em queda de rede, `/model` transacional, `/resume`, REPL com editor de linha e ledger de runs com scheduler sob lease. Entregue em dois trens de merge (#1317, 8 PRs; #1318, 10 correções com revisão adversarial em três lentes) mais o PR de release; issues que ficam abertas têm bloqueio nomeado no `TODO.md` (decisão do dono: #1247, #1254, #1261, #1272, #1295 item 1; fatias estruturais: #1225, #1226, #1227; gate manual: #1276; API externa: #1299; processo: #1228). Anterior (2026-09-15): **repo zerado (0 issues, 0 PRs, só `main`)**: M1 do Desktop Control Center (`garra desktop`, PR #1205) mergeado, cinco bumps do Dependabot em série e o épico #1181 fechado por decisão do dono com **M2–M7 rastreados na §4.1** (ADR 0021 segue como decisão de arquitetura). Anterior (2026-09-07): **release v0.3.9**: 48 issues e PRs (#933-#1012) em tres frentes — epico #944 (Terminal UX v2) fechado, lote de memoria semantica (#948-#965) fechado, e o bloco de modos (#979-#988) fechado, com a `ToolPolicy` passando a valer no executor em vez de so ser pedida no prompt. Tambem: SEC #1012 (identidade em `/v1/chat/completions` deixa de vir do chamador) e o corpo de release curado a partir do CHANGELOG (`scripts/release/notes.py`). **O repositorio ficou com zero issues abertas.** Anterior (2026-09-01): **release v0.3.5** (plan 0362 + Amendment do ADR 0015): **Garra Desktop para Linux** (`garraia-desktop-linux-x86_64.deb`/`.AppImage` via bundler Tauri, job best-effort `build-linux-desktop`; o `.deb` declara `Provides/Conflicts/Replaces: garraia` porque instala o sidecar em `/usr/bin/garraia`), **Garra Chat Bar** (barra flutuante `Ctrl+Space` que substitui o quick-chat e compartilha a sessão `parrot-desktop` com o papagaio), streaming de chunks no `/ws/parrot`, sprite do papagaio commitado + guarda `assert-ui-assets` no CI, migração argon2 0.6 / pbkdf2 0.13 / password-hash 0.6.1 (GAR-669 slice 3). Anterior (2026-08-31): **release v0.3.4** (plan 0361 + ADR 0015): primeira release a exercitar a matriz do plan 0359 (archives, MSI/NSIS, `install.ps1`) e a estrear **pacotes Linux `.deb`/`.rpm` (nfpm) + AppImage x86_64 (appimagetool)** no job best-effort `package-linux`, **binário nativo Windows ARM64** (`garraia-windows-aarch64.exe` + `.zip`, job `build-windows-aarch64`) com `garra update` e `install.ps1` resolvendo o asset novo, suíte `tests/install_ps1/platform.ps1` e casos mixed de checksum nos dois instaladores. Restam DMG notarizado e AppImage aarch64 (§4.1). Ainda em 2026-08-30: plan 0360 / ADR 0014 — `garra agents setup|status|link|rollback|web` (front-end do AgentDeck) + shim Anthropic-compatible `POST /v1/messages` para o Claude Code apontar para o gateway. Anterior (2026-08-18): cleanup total do repo (0 issues, 0 PRs, só `main`): releases **v0.3.1** e **v0.3.2** (5 binários — `garraia-linux-aarch64` pela primeira vez; cross da git + sqlx→rustls, PR #846); fix crítico LGPD no `POST /v1/me/anonymize` (plan 0354, PR #843 — retornava 500 desde junho) + job CI `auth-integration` rodando a matriz RLS em todo PR; deps consolidadas incl. wasmtime 47 (PR #844; depois 48 + MSRV 1.95 no PR #865, 2026-08-28); timeouts e2e/playwright + grupo dependabot wasmtime + Cross.toml (PR #842); h2 0.4.16 (RUSTSEC-2026-0258). **Linear descontinuado — planejamento interno migrou para o tracker interno; trigger de issues da garra-routine desativado.** Anterior (2026-05-24): docs/backlog sync: `TODO.md` criado como fila operacional obrigatória, GAR-493/ADR 0011 ✅ **Done** via PR [#492](https://github.com/michelbr84/GarraRUST/pull/492) (`95618d3`), `plans/README.md` sincronizado, e checklist GAR-603 reconciliada com evidência já presente em `Dockerfile`, `router.rs` e `docs/deployment-runpod.md`; smoke Docker/Runpod real permanece pendente. GarraMaxPower sincronizado: GAR-498 Skills MVP ✅ **Done** (PR [#488](https://github.com/michelbr84/GarraRUST/pull/488) `c65e099`), GAR-499 Agent Team MVP ✅ **Done** (PR [#490](https://github.com/michelbr84/GarraRUST/pull/490) `7e45ec5`). GAR-695 health run 23 docs ✅ **Done** via PRs [#493](https://github.com/michelbr84/GarraRUST/pull/493) / [#494](https://github.com/michelbr84/GarraRUST/pull/494). Anterior: GAR-679 SSE rate-limit per user ✅ **Done** (plan 0163); GAR-680 Audit-log of SSE chat subscriptions ✅ **Done** (PR [#463](https://github.com/michelbr84/GarraRUST/pull/463) `a972947`); GAR-496 Repo workflow seguro ✅ **Done** (PR #455 `1b7f04c`); GAR-495 ✅ **Done** (PR #453 `e5a2a08`); GAR-669 Slice 2 + GAR-500 Auto Dream ✅; GAR-372 embeddings scaffoldado; ADR 0010 Accepted + `garraia-learning`.
> **Owner:** @michelbr84
...
- [x] **Multipart upload** nativo do S3 para arquivos > 16 MiB ✅ (`put_stream` no `S3Compatible` faz `create/upload/complete` em partes de 8 MiB com abort em falha; 24 MiB em 3 partes, round-trip byte-a-byte — cobertura real via MinIO testcontainer confirmada em 2026-09-16 pela #1230, depois que a #1214 a levou para a matriz de CI: ate entao os 8 testes de `s3_integration.rs` se auto-pulavam calados porque `minio/minio` saiu do Docker Hub, e "testado contra MinIO" nao era reproduzivel em lugar nenhum. Checksum server-side por parte entrou com a #1229: SHA-256 declarado no `create`, em cada `upload_part` e no `complete`, com `GetObjectAttributes` conferindo as 3 partes em CI e um teste negativo exigindo que o servidor recuse parte com checksum errado.)
...
- [x] **Control Center -- M0 e M1** ([ADR 0021](docs/adr/0021-garraia-desktop-control-center.md), aceito em 2026-09-14, opção D; épico #1181, fechado em 2026-09-15 por decisão do dono com os milestones restantes rastreados aqui): crate `garraia-desktop-core` **sem Tauri** e dentro dos gates obrigatórios de CI (`state`/`detect`/`supervise`, PR #1195, 2026-09-14) e `garra desktop [--status] [--no-launch]` na CLI, que localiza e lança o app instalado via `locate` (instalador da plataforma > `PATH` > diretório da própria CLI, nunca o próprio executável), exit codes sysexits 0/69/70 e zero dependência de Tauri (PR #1205, 2026-09-15). Ordem de dependência dos milestones: **core → CLI → casca → abas**.
- [ ] **Control Center -- M2 (R3)**: janela principal + navegação; abas **Home** e **Desktop** (toggles do pássaro e da Chat Bar). É aqui que o gatilho S1 do [ADR 0007](docs/adr/0007-desktop-frontend.md) é reavaliado (nove abas passam do limite de "≥ 10 telas com estado compartilhado"; HTML+vanilla vs SolidJS é decisão com ADR próprio) e que entra o canal de instância única na casca — hoje `garra desktop --status` declara "execução não detectável" de propósito, e o "se já roda, foca" do ADR depende desse canal.
- [ ] **Control Center -- M3 (R4)**: aba **Agents** — detecção, status, start/stop/restart, abrir chat — como **cliente do AgentDeck** (`michelbr84/AgentDeck`, outro repositório e outro ciclo de release), sem reimplementar adapters em Rust; degrada com elegância quando ele não está instalado. `security-auditor` obrigatório. Três regras fail-closed: achar é leitura (varre `PATH` e configs, nunca executa o binário para "ver se é ele"), adicionar é decisão do usuário (detectado ≠ adicionado), executar é ação explícita com o comando visível antes de rodar.
- [ ] **Control Center -- M4 (R2)**: abas **Models/Providers** e **Integrations** sobre `/api/providers*`, `/api/mcp*`, `/api/channels` (guarda anti-CSRF + CORS fechado já entregues em #1182/#1186 como pré-requisito das abas mutantes).
- [ ] **Control Center -- M5 (R2)**: abas **Activity/Tasks** e **Logs** consolidados, por agente.
- [ ] **Control Center -- M6 (R3)**: aba **Settings** sobre `/api/settings/{schema,effective}` + `PATCH` (hoje dry-run; só `/api/diagnostics` tem teste).
- [ ] **Control Center -- M7 (R5)**: empacotamento, atualização e docs — **para e escala ao humano** (regras 15/16 do `CLAUDE.md`). Critério de "não quebrou nada" de cada merge da casca: pássaro nasce visível e `Alt+G` alterna; Chat Bar nasce no topo central, `Ctrl+Space` alterna, `Esc`/✕ oculta e a posição persiste validada contra monitor conectado; bandeja mantém os nove itens; gateway sobe como sidecar e morre junto com o app; `config.yml` e `chat-bar.json` existentes produzem a mesma tela; `cargo build --workspace --exclude garraia-desktop` continua verde. A casca Tauri segue fora do CI (validação visual manual ou runner com GTK).
- [ ] **Instaladores -- restantes**: DMG (Mac, notarizado — exige segredos Apple do mantenedor). AppImage aarch64 ✅ entregue (release.yml `package-linux`: appimagetool x86_64 + `--runtime-file` runtime-aarch64 pinnado por SHA-256, best-effort condicionado ao binário aarch64; asset `garraia-linux-aarch64.AppImage`).
...
- [ ] **docker-compose** para dev local: Postgres (`docker-compose.postgres.yml`), Ollama (`docker-compose.ollama.yml`), OTel/Jaeger/Prometheus/Grafana (`ops/compose.otel.yml`) ✅; MinIO adicionado (`docker-compose.minio.yml`, PR 2026-09-15 — bucket `garraia-dev` auto-criado).
