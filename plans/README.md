...
| 0019 | [GAR-393 Slice 3 — `POST /v1/invites/{token}/accept`](0019-gar-393-v1-invites-accept.md) | [GAR-393](https://linear.app/chatgpt25/issue/GAR-393) | 🚧 In execution (`feat/0019-gar-393-v1-invites-accept`). Slice 3 de GAR-393. Accept invite: Argon2id token verify, expiração (410), double-accept (404), member-collision (409), INSERT `group_members`. 6 tasks. Path ajustado de `:accept` para `/accept` por limitação do `matchit` no Axum 0.8 (um parâmetro por segmento). Desbloqueia `:setRole` + DELETE member (plan 0020). |
