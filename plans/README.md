...
| 0016 | [Fase 3.4 — Slice 2: AppPool + harness + authed `/v1/me` + `/v1/groups` skeleton](0016-fase-3-4-slice-2-apppool-harness-groups.md) | GAR-WS-API (destrava definitivamente GAR-391d após M4) | 🚧 **M1+M2+M3 merged** 2026-04-14 — M1 (`3d2fc66`, PR #10) `AppPool` newtype + `GARRAIA_APP_DATABASE_URL` + `AppState.app_pool` + split `RestV1State` → `AuthState`/`FullState` + router 3-way match + `/v1/groups` stubs; M2 (`a6c337d`, PR #12) gateway integration harness (pgvector testcontainer + typed pools + `build_router_for_test` + `harness_smoke`); M3 (`b139861`, PR #13) `SecurityAddon` bearer scheme + `seed_user_with_group` fixture + bundled `v1_me_authed_scenarios` (401/200-no-group/200-with-group/403/openapi-wire). **M4+M5 pendentes:** M4 real `/v1/groups` handlers (POST + GET {id}) destrava definitivamente o plan 0014 (GAR-391d app-layer matrix); M5 sweep dos review follow-ups residuais de PRs #8/#10/#12/#13 + M1/M2-carried cleanups (admin_url accessor, URL parsing, `test-support` rename, `exec_with_tenant` closure API, fail-soft `/docs/{*path}`). |
