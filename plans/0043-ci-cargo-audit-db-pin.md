...
- **v5 (2026-04-22 14:15 ET) — core2 yanked separate code path:** quarta run `24794618822` ainda vermelha com `1 denied warning found!`. Motivo: `core2 0.4.0` está yanked em crates.io registry metadata (não é advisory RUSTSEC stricto sensu) — o `--deny yanked` flag avalia o campo `yanked` do index, não consulta a advisory DB. Ignore via `[advisories].ignore` não cobre esse caminho. Como `core2` tem "all versions yanked, unmaintained" upstream (não temos fix via bump), opção pragmática: **drop `--deny yanked`** do invocation. Yanked continua visível como yellow warning no log. Swap para crate vivo fica com B-2 (rastreia pela árvore `image → rav1e → bitstream-io → core2`).
