...
| 26/09 18:30 | 50 | 8 (#1489 #1490 #1492 #1493 #1494 #1495 #1496 + trem #1491) | 0 | `87c66ec3`: 6/6 verdes; trem `982483bf` tudo QUEUED | 0 |
...
| #1409 | P1 | Web Console — show the effective mode for each WhatsApp session | L3 | principal/modo efetivo/projeto por sessao no console | `feat/1409-modo-efetivo-por-sessao` | `admin_sessions_modo_efetivo.rs` 1/1 + Playwright | PR #1498 aberta |
...
| #1415 | P1 | add a per-conversation Capability Panel in the Web Console | L3 | painel de capacidades por conversa | `feat/1409-modo-efetivo-por-sessao` | idem (`file_write` denied p/ usuario read) | PR #1498 aberta |
...
| #1422 | P1 | Web Console — show rejected WhatsApp messages safely | L3 | rejeicoes por motivo (motor puro + turno + diagnostics + API + console) | `feat/1422-rejeicoes-no-console` | `rejeicoes.rs` 6 + harness + diag + `admin_whatsapp_rejections.rs` + Playwright | PR #1499 aberta |
...
| 26/09 10:05 | **PR #1489** (L1-c, `feat/1388-access-policy-v2`, 1 commit `1bcf4fb1`) aberta contra `main`: gateway 1680, modulo 114, clippy gateway+cli limpos, fmt ok. Nenhuma issue fechada (motor; CLI/API/Web/E2E em L2) — criterios marcados na PR | `gh pr view 1489` |
| 26/09 11:30 | **L2-cli em curso** (`feat/1396-whatsapp-access-cli`, empilhada na #1489): motor unico no gateway `politica/{mutacao,impacto,auditoria}.rs` (RED 0/14 → GREEN; modulo 128/128) e CLI `whatsapp/politica.rs` + `Action::Access` + clap (`access`, `level`, `write`, `block`, `unblock`; RED 0/10 → GREEN 10/10). Docs e fragmento prontos; suite da CLI + clippy rodando | worktree `GarraRUST-l1c` |
| 26/09 12:20 | **PR #1490** (L2-cli, `71459324`): CLI 840 passed, modulo gateway 128, clippy gateway+cli limpos, fmt ok; achado no caminho: o guard `sigpipe_decide_cada_subcomando_aninhado_pelo_nome` exige cada variante nova nomeada em `sigpipe_padrao_para` (feito). **Trem `train/v046-d` → PR #1491** (#1489 + #1490) | `gh pr view 1490 1491` |
| 26/09 13:10 | Flake achado no CI da #1489 (job `clippy + tests (mcp)`): `mcp_npx_cache_recovery::stderr_pointing_outside_the_cache_deletes_nothing` com ETXTBSY (fork herda fd de escrita de outro teste). **PR #1492** serializa os 6 testes (6/6 ×3 local); adicionada ao trem d | run 36244580680 |
| 26/09 13:40 | **L2-api** (`feat/1402-whatsapp-access-admin-api`, empilhada na #1490): `visao::documento` movido para o gateway (um documento para CLI/API/console), `admin/whatsapp_access.rs` (GET/POST/audit, Channels Read/Update, audit `origem: admin_api`), diagnostics avisa `open` e secao invalida. RED (`todo!`) → GREEN: teste de integracao 1 arquivo/10 cenarios, modulo 190, CLI 10. Commit/PR em curso; entra no trem d | worktree `GarraRUST-l1c` |
| 26/09 13:45 | Worktrees dos subagentes (`agent-a485…`, `agent-a91d…`) removidas: branches mergeadas, arvores limpas | `git worktree list` |
| 26/09 14:20 | **PR #1493** (L2-api) aberta e no trem d. **L2-web**: `feat/1402-whatsapp-access-console` → **PR #1494** (pagina WhatsApp Access com preview/confirm, `identity_last4`, spec Playwright; motor `Papel`/`Remover` RED 0/2 → GREEN, modulo 130/130; API estendida 16 cenarios; clippy limpos) e no trem d | `gh pr view 1493 1494 1491` |
| 26/09 15:30 | **L2-reg (1/2) em curso** (`feat/1425-disponibilidade-de-ferramenta`, worktree `GarraRUST-reg`, base = trem d): `Tool::disponibilidade()` + `definicoes_do_turno` (as 3 listas num helper) + consulta no despacho (opcao B da #1425); `TelegramSendTool` = `not_configured`/`channel_offline`/`Disponivel` pela config viva + registry; `Denial::NoRoots` com mensagem propria e acionavel (#1418). RED (tool indisponivel entrava na lista e executava; 3 recusas identicas) → GREEN: agents lib 722/722; gateway `channel_send_tool`+`garra_status` verdes; clippy agents limpo; suite do gateway rodando | worktree `GarraRUST-reg` |
| 26/09 16:40 | **PR #1495** (L2-reg 1/2, `e61095db`): agents 722, gateway 1699, clippy limpos; no trem d (agora 6 PRs: #1489 #1490 #1493 #1494 #1495 #1492). **L2-reg 2/2 em curso** (`feat/1381-registro-de-capacidades`): `capacidades_registro.rs` (puro, 3 testes), `garra_status.capabilities` + `capabilities_means`, nota do prompt com os 5 estados (sem "ferramenta"), `tools.capabilities` no diagnostics, `GET /admin/api/capabilities`, device tools `no_devices`; verificacao rodando | worktree `GarraRUST-reg` |
| 26/09 17:30 | **PR #1496** (L2-reg 2/2, registro de capacidades): agents 723, gateway 1704, clippy limpos, fmt ok; no trem d (7 PRs). Worktree `GarraRUST-sess` criada (base = trem d) para o lote de sessao (#1379/#1424: `/project` persistente + slash no WhatsApp por principal) | `gh pr view 1496 1491` |
| 26/09 18:30 | **L2-sess em curso** (`feat/1379-projeto-por-sessao`, worktree `GarraRUST-sess`): `projetos_da_sessao.rs` (selecionar/listar/limpar/restaurar, confinado por `project_root`, persistido em `sessions.project_id`), `/project` no registro de comandos, REST `POST/GET /api/projects` persistidos, slash por principal no turno do WhatsApp (`/mode` so dono; `/project` dono+usuario; `/help` todos). 6/6 testes; alvo gateway 143/143; suite completa + clippy rodando. Decisao: **nao** adicionar mais PRs ao trem d ate o CI dele terminar (fila do Actions: 8 PRs, tudo QUEUED, head reiniciado a cada merge no trem) | worktree `GarraRUST-sess` |
| 26/09 19:20 | Retomada apos limite de uso. Estado revalidado: `main` `87c66ec3` (6/6), latest release v0.4.5 (55 assets), 50 issues, 9 PRs, 0 alertas. **PR #1497** (L2-sess, `ef3a186e`) aberta fora do trem; verificacao local em curso (clippy + suite). Trem #1491 (`982483bf`): 9 checks verdes, faltam Clippy/Test ubuntu/E2E/Playwright (fila); uniao verificada localmente em paralelo. Subagentes em worktrees a partir de `982483bf`: **lote-cli-audit-wizard** (#1414 audit dos comandos legados + #1429 wizard pos-QR) e **lote-runtime-breaker** (#1417) | `gh pr view 1491 1497` |
| 26/09 12:05 | Trem #1491: 9 checks verdes, Clippy/Test ubuntu/windows/E2E/Playwright ainda na fila; **uniao verificada localmente**: clippy 0, agents 735, gateway 1704, cli 840, fmt ok. #1497 verificada (clippy 0, gateway 1705). **L3-web (#1409/#1415)** na worktree `GarraRUST-web2`: `admin::sessoes` (principal + piso + modo efetivo + projeto sem caminho em `GET /admin/api/sessions`), `GET /admin/api/capabilities?session_id=` com portao do turno (piso ∧ teto) e `session`, pagina Sessions com colunas e painel Capabilities, spec Playwright `admin-sessions.spec.ts`. RED contra o handler antigo (`left: Null, right: "dono"`) → GREEN; admin lib 69; clippy gateway 0; fmt ok → **PR #1498** (`f1b600b3`, base = trem d). Subagentes cli/runtime ainda rodando (RED em curso, sem commit) | `gh pr view 1498`, tasks bhrp03u35/bizquov2i |
| 26/09 12:40 | **L3-rej (#1422) em curso** na worktree `GarraRUST-rej` (`feat/1422-rejeicoes-no-console`, base = trem d): modulo puro `whatsapp_linked/rejeicoes.rs` (motivos fechados `restricted_policy`/`unresolved_lid`/`blocked_user`/`channel_disabled`/`prompt_injection`/`policy_not_admitted`, `…1234` apenas, teto 50 recentes, `zerar`), `WhatsAppLinkedRuntime::{registrar_rejeicao,rejeicoes,zerar_rejeicoes}`, `PortaoDoCanal::bloqueado` + `motivo_da_recusa` puro; testes RED escritos (harness `remetente_fora_da_politica_restrita…`, tabela de motivos, diagnostics `rejeicoes_por_motivo_entram_como_contagem_sem_final`, integracao `admin_whatsapp_rejections.rs`), card no console + spec Playwright `whatsapp-rejections.spec.ts`, docs e fragmento prontos; GREEN preparado (turno, diagnostics, `documento` + reset auditado) para aplicar depois do RED | worktree `GarraRUST-rej` |
| 26/09 13:25 | **L3-rej (#1422) GREEN**: RED em tres pontos (harness `total` 0; diagnostics sem contagem; admin `rejections` Null) → GREEN 9 alvo, 271 vizinhos, 129 `whatsapp_linked::tests` (guard de log exigiu binding em vez de `as_str()` no `warn!`), integracao 1+1, clippy 0, fmt ok. De passagem: audit admin da mutacao com `(action, resource_type)` invertidos, corrigido. **PR #1499** aberta (base = trem d). **#1417 (breaker)**: subagente entregou `314dd28e`; revisao do integrador pediu correcao — `DENIAL_MESSAGE` (fora das raizes) nao e deterministica e nao pode abrir com uma recusa (vira generica, 3 iguais); aguardando commit novo. **#1414**: primeiro commit `b0c3ada4` na worktree cli2, segundo (#1429) aguardando suite da CLI. **#1420**: subagente `lote-web-doctor` iniciado (worktree `GarraRUST-doc`): motor do `doctor whatsapp` movido para o gateway + `GET /admin/api/whatsapp/doctor` + card no console | `gh pr view 1499`, task bt9lrouow |
