...
This directory contains examples for deploying GarraIA using Docker Compose.
...
This configuration runs GarraIA connected to cloud LLM providers (Anthropic, OpenAI, etc.). It uses `docs/deployment/config.basic.yml`.

```bash
docker compose -f docker-compose.yml up --build -d
```

GarraIA will be available at `http://localhost:3888`.

## Example 2: Gateway + Local Ollama

This configuration runs GarraIA alongside a local Ollama instance in the same Docker network. It uses `docs/deployment/config.ollama.yml`.
...
2. **Important:** You must pull the LLM model inside the Ollama container before GarraIA can use it.
   ```bash
...
3. GarraIA will be available at `http://localhost:3888` and will communicate with Ollama internally at `http://ollama:11434`.
...
These files are mounted to `/home/garraia/.config/garraia/config.yml` inside the container. To customize the configuration, you can edit these files or create your own and update the `docker-compose.yml` volume mapping.
