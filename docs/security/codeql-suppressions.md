...
| <a id="alert-173"></a>[#173](https://github.com/michelbr84/GarraRUST/security/code-scanning/173) | `rust/cleartext-logging` | `crates/garraia-cli/src/whatsapp.rs:668` | dismissed-false-positive | `false_positive` | **PR [#1256](https://github.com/michelbr84/GarraRUST/pull/1256) (#1238).** Mensagem de sucesso do `garra whatsapp link`: o que chega ao stdout e um **caminho de diretório**, não credencial e não PII — `<data_dir>/whatsapp/<account>/`. O `<data_dir>` sai de `AppConfig::resolved_data_dir()` (a máquina do próprio usuário) e o `<account>` é `DEFAULT_ACCOUNT` (`pub const … = "default"`, [session.rs:77](../../crates/garraia-channels/src/whatsapp_linked/session.rs)) no **único** call site de `SessionStore::for_data_dir` da CLI ([whatsapp.rs:163](../../crates/garraia-cli/src/whatsapp.rs)). O caminho impresso é, literalmente, `.../whatsapp/default/`. **Por que supressão e não correção:** o mesmo segmento de conta já aparece em outros três `println!` do comando (`blob_path()` no `status`, `archive_path()` no aviso de arquivado) e nenhum foi sinalizado; apagá-lo só neste sink calaria o scanner sem mudar nada do que ele teme, e dizer onde a sessão foi gravada é o motivo de a mensagem existir. **Deixa de valer se a conta virar dinâmica** — `for_data_dir` aceita qualquer string e a fatia do gateway chama a mesma função. O invariante está pinado por `every_session_store_in_the_cli_uses_the_constant_account` ([whatsapp/tests.rs](../../crates/garraia-cli/src/whatsapp/tests.rs)), que varre o fonte e falha se algum `SessionStore` da CLI deixar de passar `DEFAULT_ACCOUNT`: esse vermelho é o gatilho de reauditoria desta linha. ⚠️ Número a reconferir após o merge — 173 foi atribuído na análise da branch. | PR-1256 |
