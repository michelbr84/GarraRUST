...
> - **#113** (`wizard/mod.rs:769`; era `:673` até o #1180 mover o wizard): o
>   `key` vai só para `vault.set(entry, key)`; o `println!` interpola apenas
>   `entry`. A correção de 2026-08-30 procede.
...
| <a id="alert-111"></a>[#111](https://github.com/michelbr84/GarraRUST/security/code-scanning/111) | `rust/cleartext-logging` | `crates/garraia-cli/src/config_cmd.rs:222` | dismissed-false-positive | `false_positive` | **Wave 3 — cleartext.** `garraia config check` imprime presenca, nunca valor: o campo `summary.gateway_api_key_set` e `bool` e as unicas strings possiveis sao os literais `"set"` / `"not set"`. A secao sai sob o cabecalho literal `"Summary (redacted)"` (config_cmd.rs:206). CodeQL rastreia o identificador chamado `api_key`, nao o booleano que ele carrega. | GAR-491 |
| <a id="alert-112"></a>[#112](https://github.com/michelbr84/GarraRUST/security/code-scanning/112) | `rust/cleartext-logging` | `crates/garraia-cli/src/config_cmd.rs:245` | dismissed-false-positive | `false_positive` | **Wave 3 — cleartext.** Mesma superficie do [#111](#alert-111): imprime `summary.llm_providers_api_key_set.join(", ")`, que e a lista de **nomes de provider** com chave configurada (ex.: `[openai, anthropic]`), nao as chaves. Invariante de redaction documentado em CLAUDE.md (garraia-config, plan 0046). | GAR-491 |
| <a id="alert-113"></a>[#113](https://github.com/michelbr84/GarraRUST/security/code-scanning/113) | `rust/cleartext-logging` | `crates/garraia-cli/src/wizard/mod.rs:1005` | dismissed-false-positive | `false_positive` | **Wave 3 — cleartext.** `println!("  Cloud provider API key encrypted in vault (entry {entry}).")` imprime o **nome** da entrada do cofre, nunca o segredo. `cloud_secret` e `Option<(&str, &str)>` ([wizard/mod.rs:828](../../crates/garraia-cli/src/wizard/mod.rs)); o destructuring `(entry, key)` manda o `key` exclusivamente para `vault.set(entry, key)` na linha anterior, e o `println!` interpola apenas `entry`. CodeQL contamina `entry` por ele vir do mesmo tuple que `key`. **Re-audit 2026-08-30:** esta entrada apontava para `:640` e descrevia outro statement — o `eprintln!` do `vault open failed`, hoje em `:841`. O alerta vivo sempre esteve no `println!` de `:673`, que e o que [§5.3](#53--os-16-path-injection-foram-renumerados-nao-reabertos-2026-08-29) ja dizia; era o `.json` que estava errado. Ver a nota de re-audit apos esta tabela. **Reancorada 2026-09-16 (PR #1252):** o sink desceu de `:769` para `:826` porque a geracao de credencial em bind exposto acrescentou linhas acima deste ponto. Mesmo statement, mesmo sink, `sink_snippet` intocado. **Reancorada de novo 2026-09-16 (mesmo PR):** o sink desceu de `:826` para `:867` quando a credencial deixou de ser impressa e a escrita com permissao restrita entrou acima deste ponto. As duas citacoes de linha da propria justificativa tambem estavam velhas e foram corrigidas junto: a declaracao de `cloud_secret` esta em `:828` (era `:630`) e o `eprintln!` do vault em `:841` (era `:643`). Mesmo statement, mesmo sink, `sink_snippet` intocado. **Reancorada uma terceira vez 2026-09-16 (merge do #1256 com a main):** o #1256 e o #1252 reancoraram esta MESMA entrada em paralelo — aquele por 4 linhas de roteamento do `garra whatsapp`, este pela geracao de credencial. Depois do merge as duas somam, entao a linha nao e a de nenhum dos dois lados: foi RECALCULADA na arvore mesclada para `:871`. As duas citacoes internas subiram os mesmos 4: `cloud_secret` em `:828` e o `eprintln!` do vault em `:841`. Mesmo statement, mesmo sink, `sink_snippet` intocado. | GAR-491 |
| <a id="alert-115"></a>[#115](https://github.com/michelbr84/GarraRUST/security/code-scanning/115) | `rust/cleartext-storage-database` | `crates/garraia-db/src/session_store.rs:2201` | dismissed-false-positive | `false_positive` | **Triagem 2026-08-30.** O que chega ao banco e um hash, nunca o token: `validate_session_token` liga `params![hash_session_token(token)]`, e `hash_session_token` ([session_store.rs:1612](../../crates/garraia-db/src/session_store.rs)) e SHA-256 hex de mao unica. O lado da escrita declara o invariante em `session_store.rs:1634` — *"Only the hash is persisted; the raw token goes to the caller and is never written to disk"* — e ha migracao que reescreve tokens legados em claro, detectados por `length(token) <> 64 OR token GLOB '*[^0-9a-f]*'` (`session_store.rs:347`). CodeQL trata `token: &str` como credencial fluindo para uma query, sem modelar o hash como transformacao irreversivel. **Reancorada 2026-09-06 (PR #1007):** o sink desceu de `:1380` para `:1653` porque `set_agent_mode` ganhou distincao entre modo escolhido e deduzido (#988), o upsert passou a mesclar metadado (#1008) e o modo customizado ganhou escopo de usuario (#986) — 273 linhas acima no total. Mesmo statement, mesma funcao, mesmo sink. **Reancorada 2026-09-09 (PR #1114):** o sink desceu de `:1653` para `:1674` porque o patch de metadado nao-objeto na criacao de sessao (#1102) inseriu 21 linhas em `SessionStore`, todas acima deste ponto. **Reancorada 2026-09-15 (PR #1224):** o sink desceu de `:1674` para `:1677` porque a migration da tabela `agent_runs` (ledger duravel de runs, P1 do gap analysis) inseriu 3 linhas no inicio de `run_migrations`. Mesmo statement, mesma funcao, mesmo `sink_snippet`. **Reancorada 2026-09-20 (PR #1309):** o sink subiu de `:1677` para `:1669` e o `sink_snippet` mudou de `.prepare(&sql)` para `.prepare(` porque a nota lateral da #1247 reescreveu a query: o SQL montado por `format!` virou literal estatico, com o timeout idle entrando por bind `?2`. Mesmo statement, mesma funcao `validate_session_token`, mesmo invariante — o que chega ao banco segue sendo so o hash, agora em `session_store.rs:1681`. Nao ha mais SQL dinamico neste caminho. | GAR-491 |
| <a id="alert-144"></a>[#144](https://github.com/michelbr84/GarraRUST/security/code-scanning/144) | `rust/cleartext-transmission` | `crates/garraia-channels/src/signal/mod.rs:228` | dismissed-false-positive | `false_positive` | **Triagem 2026-08-30, reancorada neste PR.** O sink (`:228` aqui; o alerta vivo aponta `:279`, emitido contra a main antes de este PR encolher os sends para wrappers) esta **dentro** de `connect()`, depois do guard: `connect()` chama `ensure_url_vetted()?` antes de qualquer IO, e o `?` aborta se a URL for recusada. O `config` e clonado **antes** do `tokio::spawn` e `SignalConfig` nao tem mutabilidade interior, entao a URL do loop e o mesmo valor imutavel ja validado. CodeQL nao liga o clone a validacao anterior. Auditado por `security-auditor`. Este PR reforca a justificativa: o POST de resposta dentro do mesmo spawn passou a chamar `api::send_text`, que roda o guard — antes ele dependia so da ordem de execucao. Contraste com o **#143**, mesma regra e mesmo arquivo, que **nao** entrou neste ledger: era codigo morto (`receive_messages`, `#[allow(dead_code)]`, zero call sites) e foi **removido**, fechando como `Fixed`. | GAR-491 |
...
| <a id="alert-176"></a>[#176](https://github.com/michelbr84/GarraRUST/security/code-scanning/176) | `rust/cleartext-logging` | `crates/garraia-cli/src/whatsapp.rs:1358` | dismissed-false-positive | `false_positive` | **PR [#1256](https://github.com/michelbr84/GarraRUST/pull/1256) (#1238).** Mensagem de sucesso do `garra whatsapp link`: o que chega ao stdout é um **caminho de diretório**, não credencial e não PII — `<data_dir>/whatsapp/<account>/`, com `<data_dir>` saindo de `AppConfig::resolved_data_dir()` (a máquina do próprio usuário). A entrada faz **duas** afirmações, e cada uma tem prova própria. **(1) O caminho não sai do data dir**, porque `for_data_dir` devolve `Result` e recusa (`SessionError::InvalidAccount`) qualquer conta que não seja um segmento `[A-Za-z0-9_-]{1,64}` — allowlist que fecha `..`, `/`, `\`, `.`, vazio, NUL e espaço de uma vez. **E `for_data_dir` é o único construtor que existe fora da crate**: a irmã `SessionStore::new`, que aceita qualquer `PathBuf` e não valida nada, é `pub(crate)` desde a revisão R4 — quem impede a CLI de contorná-la é o rustc, não a disciplina de quem escreve o call site. Regressão da *declaração*: `the_store_exposes_a_single_validating_constructor` ([source_scan.rs](../../crates/garraia-channels/src/whatsapp_linked/source_scan.rs)), que fica vermelho no dia em que `new` voltar a ser `pub`. Regressão: `an_account_that_escapes_the_data_dir_is_refused_before_any_write` ([session.rs](../../crates/garraia-channels/src/whatsapp_linked/session.rs)), que no ramo em que a validação aceitasse **grava** a sessão cifrada e afirma a contenção no data dir. **(2) O segmento impresso é a constante `DEFAULT_ACCOUNT`** — um número de telefone casaria a regra de (1) e seria PII no stdout, então validar não basta aqui. Regressão: `every_session_store_in_the_cli_uses_the_constant_account` ([whatsapp/tests.rs](../../crates/garraia-cli/src/whatsapp/tests.rs)), que varre **a árvore inteira** de `crates/garraia-cli/src` lida do disco e exige que o segundo argumento de cada `for_data_dir` seja literalmente `DEFAULT_ACCOUNT`, lendo o argumento até o parêntese que fecha a chamada — `account.unwrap_or(DEFAULT_ACCOUNT)` é reprovado, e o parser tem teste próprio. ⚠️ **Reauditada 2026-09-16 (revisão R4):** a versão anterior desta linha afirmava que o invariante estava pinado por um scan de janela de 160 caracteres num arquivo só; ele aprovava conta dinâmica com a constante como *fallback* e não enxergava call site em outro arquivo, e o `for_data_dir` não validava nada — o path traversal estava fechado por acidente. Guarda textual → validação estrutural. **Por que supressão e não correção:** o mesmo segmento já aparece em outros **dois** `println!` do comando — `store.blob_path()` no `status` e `store.archive_path()` no aviso de arquivado — e nenhum dos dois foi sinalizado; apagá-lo só neste sink calaria o scanner sem mudar nada do que ele teme, e dizer onde a sessão foi gravada é o motivo de a mensagem existir. **Deixa de valer** se a CLI passar a escolher conta em runtime: é o teste (2) que fica vermelho nesse dia. **Reancorada 2026-09-16 (PR #1256):** o sink desceu de `:631` para `:807` porque as revisões R3/R4 acrescentaram acima deste ponto o tratamento de erro do `for_data_dir`, o `ArchiveGuard` e a extração de `link_paired` — mesmo statement, `sink_snippet` intocado. **Reancorada de novo 2026-09-17 (PR #1256, rodada 4):** desceu de `:807` para `:994` — a rodada acrescentou acima dele o prazo do handshake, a saída injetável do `ArchiveGuard` e o comando `garra whatsapp restore`. Mesmo statement, `sink_snippet` intocado. **Reancorada de novo 2026-09-17 (PR #1256, rodada 5):** desceu de `:994` para `:1017` — a rodada acrescentou acima dele a prova de que o blob restaurado abre antes de `restore` ligar o canal. Mesmo statement, `sink_snippet` intocado. **Reancorada de novo 2026-09-17 (PR #1256, rodada 6):** desceu de `:1017` para `:1018` — a rodada inverteu a ordem daquela prova, que agora roda ANTES de mover o arquivado. Mesmo statement, `sink_snippet` intocado. **Reancorada de novo 2026-09-17 (PR #1256, rodada 9):** desceu de `:1018` para `:1067` — entrou acima dele o `needs_a_terminal`, a mensagem que `link` e `cloud` passaram a imprimir sem terminal, no lugar de repetirem o texto do menu e sairem 0. Linha **recalculada** nesta arvore, nao copiada. Mesmo statement, `sink_snippet` intocado. **Reancorada de novo 2026-09-17 (PR #1256, rodada 10):** desceu de `:1067` para `:1070` — entrou acima dele o pulso `PairUi::connecting`, que da sinal de vida enquanto a ponte esta muda antes do primeiro QR. Linha **recalculada**, nao copiada. Mesmo statement, `sink_snippet` intocado. **Reancorada de novo 2026-09-17 (PR #1265, fatia D):** desceu de `:1070` para `:1107` — o merge da fatia D entrou acima dele a integração de saúde do `garra whatsapp status`: o `use` do módulo `health`, `DiskFacts::read` + `classify`, o bloco de `MissingDependencies` e a função `print_next_step`. Linha **recalculada** nesta árvore com grep, não copiada. Mesmo statement, `sink_snippet` intocado. ⚠️ Número a reconferir após o merge — 173 foi atribuído na análise da branch. **Reapontada 2026-09-25 (#173 → #176):** confirmado depois do merge da v0.4.5 — o GitHub abriu o alerta **#176** para o mesmo sink (span `1356-1358`, sink em `:1358`; o `println!` virou multi-linha com o rustfmt) e o #173 ficou dispensado numa duplicata aposentada. Reapontamento feito pelo `codeql-rekey-ledger.py`, que passou a casar a linha do sink **dentro do span** do alerta, como o `codeql-reapply-dismissals.sh` já fazia. Mesmo statement, `sink_snippet` intocado. | PR-1256 |
| <a id="alert-174"></a>[#174](https://github.com/michelbr84/GarraRUST/security/code-scanning/174) | `rust/path-injection` | `crates/garraia-channels/src/whatsapp_linked/bridge.rs:639` | dismissed-false-positive | `false_positive` | **PR [#1315](https://github.com/michelbr84/GarraRUST/pull/1315) (#1238), reauditado no corte da v0.4.3.** Falso-positivo da mesma classe do #173: o `dir` de `deps_installed` (e de `materialize`/`npm_ci`, mesma origem) é o diretório do bridge Node, derivado de `AppConfig::resolved_data_dir()` — `config.data_dir` do `config.yml` do operador ou o diretório de config padrão — pelas duas únicas fontes de produção: `bridge_dir(&config.resolved_data_dir())` no gateway ([bootstrap/whatsapp_linked.rs](../../crates/garraia-gateway/src/bootstrap/whatsapp_linked.rs)) e `ctx.bridge_dir()` na CLI ([whatsapp.rs](../../crates/garraia-cli/src/whatsapp.rs)). Nenhum byte de request, de mensagem de canal ou de tool call de LLM alcança esse caminho; quem controla o `config.yml` já controla o processo. O CodeQL não distingue operador local de remetente remoto — a limitação registrada nos 16 `rust/path-injection` do GAR-490 (#67–#82). **O componente que era corrigível foi corrigido:** o nome de cada asset (trait `BridgeAssets`, público para fontes futuras) passa por allowlist de segmento único `[A-Za-z0-9._-]{1,255}` **antes** de qualquer efeito colateral (`validar_nome_de_asset`, PR #1315); regressão `materialize_recusa_nome_de_asset_que_escapa_do_dir` ([bridge.rs](../../crates/garraia-channels/src/whatsapp_linked/bridge.rs)) cobre `../escapou`, `/etc/passwd`, `a\b`, `..`, `sub/dir.mjs` e NUL e afirma que o diretório de destino nem nasce. Quando a #1315 entrou em `main` (`47dea8b`) o alerta **migrou** do sink `dir.join(file.name)` de `materialize` para `dir.join("node_modules")` de `deps_installed` — prova de que o dado marcado é o `dir`, não o nome. **Deixa de valer** se o diretório do bridge (ou `data_dir`) passar a vir de request, canal ou LLM: nesse dia a supressão cai e o caminho precisa de canonicalização + confinamento, como o `FileJail` das file tools (#1244). | PR-1315 |
| <a id="alert-178"></a>[#178](https://github.com/michelbr84/GarraRUST/security/code-scanning/178) | `rust/path-injection` | `crates/garraia-channels/src/whatsapp_linked/bridge.rs:629` | dismissed-false-positive | `false_positive` | **PR [#1373](https://github.com/michelbr84/GarraRUST/pull/1373) (v0.4.5).** Mesma classe e mesmo diretório do #174: `std::fs::metadata(&stamp)` em `deps_pending` lê o carimbo de dependências (`DEPS_STAMP_FILE`, nome constante) dentro do diretório do bridge Node que o `prepare` recebe — `PreparoDaPonte::embutido(paths.bridge_dir)` no gateway e `ctx.bridge_dir()` na CLI, ambos derivados de `AppConfig::resolved_data_dir()` do `config.yml` do operador. Nenhum byte de request, canal ou LLM alcança o caminho; a leitura é só `stat` e no máximo os 7 bytes do marcador `pending`. **Deixa de valer**, junto com o #174, se o diretório do bridge passar a vir de request, canal ou LLM. | PR-1373 |
| <a id="alert-177"></a>[#177](https://github.com/michelbr84/GarraRUST/security/code-scanning/177) | `rust/path-injection` | `crates/garraia-channels/src/whatsapp_linked/bridge.rs:631` | dismissed-false-positive | `false_positive` | **PR [#1373](https://github.com/michelbr84/GarraRUST/pull/1373) (v0.4.5).** Mesma classe e mesmo diretório do #174: `std::fs::read(&stamp)` em `deps_pending` lê o carimbo de dependências (`DEPS_STAMP_FILE`, nome constante) dentro do diretório do bridge Node que o `prepare` recebe — `PreparoDaPonte::embutido(paths.bridge_dir)` no gateway e `ctx.bridge_dir()` na CLI, ambos derivados de `AppConfig::resolved_data_dir()` do `config.yml` do operador. Nenhum byte de request, canal ou LLM alcança o caminho; a leitura é só `stat` e no máximo os 7 bytes do marcador `pending`. **Deixa de valer**, junto com o #174, se o diretório do bridge passar a vir de request, canal ou LLM. | PR-1373 |
**Total**: 30 entries (5 from GAR-491 Wave 2 + 16 from GAR-490 Wave 1 PR A +
1 from PR [#869](https://github.com/michelbr84/GarraRUST/pull/869) + 5 from
Wave 3 cleartext + 2 da triagem de 2026-08-30, #115 e #144 + 1 do PR
[#1256](https://github.com/michelbr84/GarraRUST/pull/1256)).

> **Correção de contagem, 2026-09-16 (PR #1256).** A `main` declarava **31
> entries** com **29 linhas** na tabela, e a parcela "GAR-491 Wave 2" dizia 6
> onde há 5 — a soma nunca fechou. Este PR acrescentou uma entrada (#173) e
> corrigiu a declaração para 30, que é o número que a tabela sustenta
> (5 + 16 + 1 + 5 + 2 + 1 = 30). O `31 → 30` no diff, portanto, **não** é uma
> entrada removida: é a soma passando a bater. Registrado porque num ledger de
> alertas de segurança dispensados uma contagem que muda calada é exatamente a
> espécie de número que ninguém reconfere.
> **Sink novo a esperar depois do merge, 2026-09-17 (PR #1256, rodada 6).**
> `crates/garraia-channels/src/whatsapp_linked/session.rs:694` ganhou um
> `tracing::warn!(caminho = %tmp.display(), …)` — a rodada 5 o acrescentou
> para que a falha ao triturar o temporário da sessão deixasse rastro em vez
> de sumir em silêncio. **Ele tem a mesma forma do #173**: o que vai para o
> log é um *caminho*, e o conteúdo (o blob cifrado) nunca. O ledger foi
> reancorado só para o #173, porque é o único alerta que a análise da branch
> emitiu até aqui; a análise pós-merge roda sobre outra base e **pode** abrir
> um alerta `rust/cleartext-logging` novo neste segundo sink. Fica registrado
> aqui para que ele chegue como coisa prevista, com a justificativa já pronta,
> e não como surpresa a ser despachada às pressas. Quem o triar: o argumento é
> o mesmo do #173 — caminho sob o data dir da própria máquina, conta fixa em
> `DEFAULT_ACCOUNT` — e a diferença é que aqui nem o segmento de conta é
> escolhido em runtime, porque `tmp` é derivado de `blob_path()`.
...
| Regra | # | Local | Leitura |
|---|---|---|---|
| `rust/cleartext-logging` | #111, #112 | `config_cmd.rs:213,236` | Imprime `"set"`/`"not set"` e a **lista de nomes** de provider com chave — nunca o valor. É o invariante de redaction do plan 0046. |
