...
- Status: Proposed
- Date: 2026-02-16
- Owners: GarraIA maintainers

## Context

GarraIA is a Rust rewrite of OpenClaw and aims for low-resource reliability and simple deployment.
...
GarraIA will use a **Rust core + polyglot connector** model:
...
The current protocol skeleton is defined in `crates/garraia-channels/src/protocol.rs`.
...
2. Implement connector supervisor in `garraia-gateway` (start/stop/health/restart).
